/**
 * Module 07: Content - Database Utility
...
const possibleEnvPaths = [
  ...
  path.resolve(process.cwd(), '../../.env'),     // From packages/07-content
];
...
if (!envLoaded) {
  console.warn('[Content] Warning: Could not load .env file from any expected location');
}
...
export function getPool(): Pool {
  if (!pool) {
    ...
    console.log('[Content] Database connection pool created');
  }
...
export async function closePool(): Promise<void> {
  if (pool) {
    await pool.end();
    pool = null;
    console.log('[Content] Database connection pool closed');
  }
}
